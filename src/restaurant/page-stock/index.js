...
const FormAddStockItem = () => {
  ...
  const { sku, description, caseQty, packQty, itemQty, selectedUnit } =
    formState;

  const readDataFromStorage = () => {
    AsyncStorage.getItem("@StockItem").then((data) => {
      if (data) {
  ...
  const storeDataToStorage = (data) => {
    AsyncStorage.setItem("@StockItem", JSON.stringify(data))
      .then(() => {
        AsyncStorage.getItem("@StockItem")
          .then((data) => {
            if (data !== null) {
              return data;
            } else {
          ...
          .then((data) => {
            console.log("StockItem getdata", typeof data && JSON.parse(data));
            setFormState((prev) => ({
              ...prev,
              ...JSON.parse(data),
            }));
      ...
      .catch((err) => {
        console.log("StockItem error", err);
      });
  };
  ...
  return (
    <VStack>
      <HStack p="4">
        ...
        <Button
          variant="outline"
          leftIcon={<Icon as={Ionicons} name="options" size="sm" />}
        />
      </HStack>
      <ScrollView
        style={{
